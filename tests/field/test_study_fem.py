...
produce *different* thresholds, so the sweep can finally tell them apart.
"""
...
def test_fem_study_distinguishes_two_diameters():
    ...
    differ."""
    controls = {
        "diameters_um": [10.0, 30.0],  # a 3x diameter gap, so the effect should be clear
        "pitches_um": [60.0],
        "arrangement": "hex",
    ...
    # the fields genuinely differ, so the thresholds do too, and the flat-frontier bug
