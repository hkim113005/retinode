...
- **Configuration** (or *stimulus*) is the *current delivery*: which electrodes source and which return, with what weights and waveform, over a fixed geometry. Changing configuration is cheap, because it is a weighted sum over an already-solved field.
...
A single lint rule, that `engine/` may not import from `api/` or `app/`, is worth a CI check, because this one boundary is what makes the Phase 7 re-skin a re-skin and not a rewrite.
...
- **Tier 1, analytical (point or disk source).** For a point source in an infinite homogeneous medium, `Ve(r) = I / (4πσr)`; a finite disk electrode uses the standard disk-source potential (the equipotential-disk solution) so near-field values are not singular. Fast enough for the live preview and for prototyping. This is the workhorse of the summer. *(As built, the tier is a regularized point source rather than the equipotential-disk solution: `1/r` from the electrode centre, with `r` floored at the electrode radius. Beyond that radius two diameters give identical potentials, so a diameter sweep is flat. See [phase-8-findings.md](phase-8-findings.md).)*
...
- **Configuration sweeps on a fixed array** reuse one transfer matrix `A` and cost only a weighted sum plus threshold searches, which is cheap enough to run live. A steering sweep over hundreds of weight vectors on a fixed array is a Tier-1 interactive operation.
- **Physical-geometry sweeps** recompute the field per geometry and are expensive, so they run as batched, parametric FEM jobs, coarse-to-fine, optionally with a **surrogate model**: a Gaussian-process or similar emulator over the selectivity score as a function of geometry parameters, proposing where to sample next once enough points exist.
...
**The data contract for a view is small and explicit.** A field view needs the query-point grid, the `Ve` array, the electrode outlines, and the soma/axon overlays; an activation view needs, per amplitude step, the set of firing compartments and the target flag; a scorecard needs the evaluator's scalar outputs and badges. Each is a plain JSON payload the engine already computes, so the same contract serves the Phase-2 dashboard and the Phase-7 React client unchanged: the migration re-skins the renderer, not the data.
...
- **Early, usable (Phase 2).** A Python-native dashboard (Dash, Panel, or Streamlit, leaning Dash and Plotly for interaction fidelity) that calls the engine directly and gives forms plus Plotly field, activation, and Pareto views. Minimal frontend effort, good enough to validate the UX and be genuinely usable on the fast analytical tier.
...
Extend the electrode model from flat disks pinned to the array plane to (a) arbitrary 2D outlines (square, polygon), (b) true 3D electrode bodies, both parametric primitives (pillars, wells, frustums, penetrating tips) and imported CAD (STEP/BREP), and (c) 3D placement of whole arrays into tissue: position, orientation, and per-electrode insertion depth, so an array of mixed flat and penetrating electrodes can be planted into the retina and its interaction with the neuron population simulated. The conductive domain becomes the tissue slab with the electrode bodies subtracted out; a neuron cannot occupy an electrode body, so cell/electrode overlap is resolved by an explicit policy (reject the conflict, or opt-in displace the affected compartments), and near-contact is flagged as leaving the passive-probe regime. All behind the transfer-matrix contract, so the cable/NEURON biophysics is unchanged; validated with the Phase-4 machinery (known-answer, convergence, second-solver agreement); shaped and 3D electrodes are FEM-only. **Done when** a user can define a 3D electrode array, plant it into tissue at a chosen pose and depth, solve the tissue-minus-electrodes field with overlap resolved, and get a validated selectivity/safety score for the RGC population. This turns the tool from an array-layout comparator into a 3D design-space explorer. See [phase-6-plan.md](phase-6-plan.md).
...
> **Execution note (this roadmap is forward-looking; the phase docs are authoritative).** In practice the geometry study, the Pareto frontier, and the Validation + Candidates dashboards shipped in **Phase 7** ([`phase-7-plan.md`](phase-7-plan.md), S4/S6). The delivered **Phase 8** ([`phase-8-plan.md`](phase-8-plan.md)) became *closing Phase 7's deferrals and the defect they hid*: the activation and threshold plots, trajectory sensitivity, and forcing the geometry study onto the FEM tier, because the analytical point source cannot see electrode diameter. The trust panel and the Pareto machinery this entry describes are delivered. The *design finding* itself is not: P9 ships the FEM geometry-distinction demo as an honestly-labelled capability demo, and producing a novel design finding remains future work ([`phase-9-plan.md`](phase-9-plan.md)).
...
| 10 | Sep 14–20 | (buffer) | Buffer, write-up, demo, and lab hand-off; tag a summer release | Summer deliverable complete |

The critical, unmovable milestones are the four gates. If a week slips, the buffer absorbs it; if more than the buffer slips, the cut list in §24 says what to drop, in order. The tool is still a complete story at whichever gate it reaches: Gate 2 alone is a usable design tool, and Gate 3 is the validated testbed that justifies the whole project.
...
**Reproduction mismatch.** A published result might not reproduce within tolerance, either because of a genuine modeling gap or because the paper's conditions were misread. Mitigation: treat each reproduction as an experiment with a written-down expected quantity and tolerance. A near-miss is a finding to understand and document, not a failure to hide, and the Validation screen shows the current value against the target either way.

## 23. Decision gates (go / no-go)

Each gate is a go/no-go with a defined fallback, so a slip is a decision, not a scramble:

- **Gate 0 (end Wk 2): spec frozen, analytical field validated.** *Go:* start NEURON. *No-go:* the spec is still churning, so stop and finish it. Nothing downstream is safe until it holds.
- **Gate 1 (end Wk 5): the pipeline produces defensible thresholds.** *Go:* build the dashboard. *No-go:* thresholds do not match the literature, so stay on the cable engine and adopt a reference channel implementation. Do not build UI over numbers you do not trust.
- **Gate 2 (end Wk 6): a non-coder can use it.** *Go:* start reproductions. *No-go:* the UX blocks a naive user, so fix the smallest set of blockers only. Do not gold-plate.
- **Gate 3 (end Wk 9): three reproductions green.** *Go:* the summer deliverable is done; write up and, if time remains, begin Phase 4 FEM prototyping. *No-go:* one reproduction misses, so ship the two that pass and document the third as an open finding. The tool is still a validated testbed.
...
14. The FEniCS Project / DOLFINx (FEniCSx): open-source finite-element solver, the primary FEM backend.
15. Schöberl J. NGSolve: finite-element library, the FEM cross-check backend.
...
- **Default array template.** Which template ships as the default? Matching the lab's 512-electrode arrays (10 µm electrodes, 30–60 µm pitch) used in the results being reproduced is the obvious candidate.
