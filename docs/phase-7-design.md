...
This extends, and never replaces, the established visual language (master plan §13):
...
completion**, a distinct alert for safety, hairline structure, glass cards, and
dark-mode awareness. Restraint everywhere except one signature moment. The tokens are
already in `app/assets/style.css` and are locked.

> **Where the tokens live now.** P7 S8 retired the Dash app, so `app/assets/` is
> gone. The same locked palette carries forward in the React client's
> `app/web/src/ui/tokens.css`.
...
- **The expert** (the lab, and later you): wants density, keyboard speed,
  provenance, and the ability to audit any number.
...
- **The amplitude dial drives everything.** A single horizontal control, styled like
  ...
  *watch-it-fire* moment. Dragging down un-fires them. The dial is the page's centre
  of gravity.
...
- **Isopotential contours are labeled.** The cool `Ve` heatmap underlays the cells,
  and contour lines carry their mV value inline: colour for the gestalt, numbers for
...
- **Tier is a texture, not just a badge.** An **analytical** result is drawn slightly
  *draft*: hairline contours, a touch desaturated, like a pencil sketch. An **FEM**
  ...
  canvas **settles** from draft to inked in one quiet transition, which is the only
  "job done" signal you need.
- **Where FEM disagreed with analytical, the canvas says so.** A small "Δ vs
  analytical" marker sits at the point of largest divergence, so the upgrade in
  accuracy is legible, not silent.
...
- **A signed-difference field.** Overlay two configs as one heatmap of *where config
  A's field beats B's*, so "local return beats monopolar" becomes a shape you see, in
  a diverging cool/warm scale centred on zero.
- **A delta column.** Scorecards align with a **Δ column**: Δ selectivity, Δ
  threshold, Δ safe-window, each signed and coloured. The win is a number you read,
...
- **Brush selects a family, and the selection flows to Candidates.** Drag a box on the
  ...
  portable object.
- **Axes speak human.** "µA to fire the target" against "bystanders also firing,"
  not raw metric names. The cost estimate shows *before* you launch (master plan §9),
  and the table fills incrementally as jobs return.
...
This is how the tool stays "optimal for information": many designs visible at once,
drill-down on demand, no modal reloads.
...
can glance at geometry intuition without leaving the 2D truth. Click to expand it
full-bleed; it collapses back. The loupe is where Phase 6's 3D geometry becomes
visible without stealing the page.

> **Status (P7 S5, corrected in P8).** The loupe ships, but it draws flat disks
> only. The view contract carries `{x_um, y_um, radius_um}` per electrode, with no
> depth, rotation, or body, so bodies, tilt, CAD, and overlap flags are **not**
> rendered. Delivering them needs a contract that carries a body and a surface that
> authors one. See [phase-8-plan.md](phase-8-plan.md).
...
- Every view carries a small **◇ state chip** with its short hash. Click it and the
...
- Hovering the chip opens a **provenance peek**: the exact spec inputs and result
  lineage behind this number, so a sceptic can audit any figure in two moves without
...
(`mesh: auto · σ: homogeneous · bystanders: 6`). You always know what you are *not*
...
the surface, so the newcomer never sees it and the expert never touches the mouse.
...
- **Sparkline everything inline:** thresholds, the activating function along an axon,
  charge density, and the biphasic waveform (a tiny scope trace in the Stimulus
...
- **The scorecard is one dense, scannable band**, not a stack of stats: the operating
  ...
  and the safety ceiling as tick marks. That is the whole selectivity story in one
  line.
...
- **S7 (charts/export)** ships labeled contours, sparklines, and figure-quality
  vector and raster export of every plot.
- **S8 (polish)** ships the ⌘K spine, teaching empty and error states,
  reduced-motion support, and the one-vocabulary audit.
