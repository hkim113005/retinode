...
without manual bookkeeping, every result traceable; **and** (app, 2b) someone who
is not us can design an array, set a configuration, and read a selectivity result
...
| # | Decision | Choice |
|---|---|---|
| P-D1 | Track | **Infra first, then app.** Build the store + sweep engine, then a dashboard (Phase 2b) that consumes it — so the UI has real sweep/cache capability to show, not single evaluations. |
| P-D2 | Store format | **parquet + HDF5** (the §10 layout): parquet for scalar result metrics, HDF5 for `A` matrices, JSON sidecars for nested/curve fields. Adds `pandas`/`pyarrow`/`h5py` (none installed yet). |
| P-D3 | Transfer-matrix reuse | **Do it now.** A per-placed-cell `SolvedField` (the transfer matrix over the cell's segments) is computed once and reused across every config and amplitude in a fixed-array sweep. This is the whole point of the geometry/config split. |
| P-D4 | Concurrency | **Synchronous.** Async/resumable jobs, cluster, and cloud adapters are Phase 5. |
| P-D5 | Sweep axes | **Configuration only:** steering weights, waveform shape, return mode — never amplitude (threshold search already sweeps it), never geometry (Phase 5). |
| P-D6 | `field_key` correction | Extend `field_key` to include the **query points / cell placement**: the Phase-1 seed keyed only (backend, array, conductivity), but a per-cell `A` cache is only correct when placement is in the key. |

## Module layout

```
engine/cable/
  solved.py     SolvedField: A cached over a placed cell's segments;      [P1]
                compute_ve/threshold search reuse it instead of rebuilding
engine/store/
  keys.py       field_key + result_key (Phase 1; field_key extended P1)   [P1]
  fields.py     HDF5 transfer-matrix cache, keyed by field_key            [P2]
  results.py    parquet result store + JSON sidecars, keyed by result_key [P2]
  project.py    Project: on-disk workspace (open/create, put/get, index)  [P2]
  provenance.py append-only run log (one record per run)                  [P3]
engine/study/
  sweep.py      configuration sweep over a fixed array + ranking/Pareto   [P4]
  cost.py       pre-sweep cost estimate from a one-eval benchmark         [P5]
app/            Python dashboard (Dash/Plotly) over the engine            [P2b]
```

## Ordered steps

- **P1 — Solved-field reuse (the efficiency core).** Introduce `SolvedField`
  ...
  the query points (P-D6). **Done when** a multi-config sweep builds each `A`
  once (call-count spy) and the reused path is bit-identical to the recompute
  path.
- **P2 — The project store (on-disk, content-addressed).** The §10 layout:
...
- **P3 — Provenance.** Append-only `provenance.log`, one record per run: spec
...
- **P4 — Configuration-sweep engine.** `sweep(array, patch, conductivity,
  configs, *, store, off_target_set, backend)` evaluates each config (reusing P1,
  caching via P2, logging via P3, skipping cached `result_key`s) and returns the
  collected `EvaluationResult`s plus a ranked shortlist over the usable operating
  window (margin/ratio) and a Pareto helper (selectivity vs. safety margin).
  Config generators: an explicit list plus steering-weight and waveform-shape
  helpers. **Done when** a sweep produces a ranked shortlist reusing `A` and
  skipping already-cached results.
- **P5 — Cost estimate (minimal).** `estimate_sweep_cost(n_configs, n_cells,
...
- **Fast (no NEURON, every push):** store round-trips (result/field put→get,
  ...
  determinism and placement-sensitivity (P-D6); sweep orchestration and ranking
...
- **App (2b):** a headless smoke render of each screen plus a scripted
...
the Pareto helper (keep flat ranking). Then drop the cost estimate. Never cut P1
(reuse) or the provenance record — they are why the results are cheap and
trustworthy.
